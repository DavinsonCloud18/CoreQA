...
function updateModal(filePath) {
  ...
  const oldHeader = `<div className="flex justify-between items-center mb-4 shrink-0">
  ...
  if (content.includes('Select Modules')) {
     content = content.replace(oldHeader, newHeader);
  }

  // Also update the module row to show "Included" badge
  const oldRow = `<span className="text-white font-medium">{m.name}</span>
                      </div>
...
console.log('UI updated for modals');
