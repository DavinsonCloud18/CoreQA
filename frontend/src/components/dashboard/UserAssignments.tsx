...
export function UserAssignments({ assignments }: { assignments: any[] }) {
  ...
  if (!assignments || assignments.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 bg-black/20 rounded-2xl border border-dashed border-white/10">
  ...
  return (
    <div className="space-y-0 max-h-[420px] overflow-y-auto custom-scrollbar">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 mb-2 sticky top-0 z-20 bg-slate-900/80 backdrop-blur-sm rounded-lg border border-white/5">
      ...
      {assignments.map((assignment, i) => {
        ...
        return (
          <div key={assignment.user.id}>
            ...
            {/* Expanded Detail */}
            {isExpanded && (
              <div className="ml-11 mr-3 mb-2 mt-1 space-y-2 animate-in fade-in slide-in-from-top-1 duration-150">
                {Object.entries(sessionGroups).map(([sessionName, moduleNames]) => (
                  <div key={sessionName} className="bg-black/30 rounded-lg p-2.5 border border-white/5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {moduleNames.map((modName, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] font-semibold text-white/75 bg-white/5 hover:bg-indigo-500/20 hover:text-indigo-200 px-2 py-1 rounded-md border border-white/10 hover:border-indigo-500/30 transition-colors cursor-default"
                        >
                          {modName}
                        </span>
                      ))}
